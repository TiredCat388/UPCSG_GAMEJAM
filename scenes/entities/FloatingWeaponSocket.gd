...
@export var ease_out_fraction: float = 0.05
...
@export var scale_ease_out_fraction: float = 0.1
...
func _process(delta):
    if not is_attacking or weapon_instance == null:
        reset_weapon()
        return
    ...
    scale = compute_scale(progress)

    update_position_and_rotation(parent_center, radius)
...
# --- Start attack ---
func first_attack():
    if is_attacking or weapon_instance == null:
        return
    is_attacking = true
    attack_angle = 0
